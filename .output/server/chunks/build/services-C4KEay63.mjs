...
import { u as useServices, _ as _sfc_main$2, a as useStatusStore, P as PRESET_SERVICES } from './index-DzaJy2UO.mjs';
import { defineComponent, ref, withCtx, openBlock, createBlock, createVNode, unref, reactive, watch, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrRenderClass, ssrRenderAttr, ssrInterpolate, ssrRenderTeleport, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual } from 'vue/server-renderer';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const ADAPTERS = [
      ...
      { value: "auto", label: "Auto-d\xE9tection" }
    ];
    const defaultForm = () => ({
      name: "",
      url: "",
      ...
      (open) => {
        ...
        if (open) {
          if (props.editing) {
            ...
            form.body = (_a = props.editing.body) != null ? _a : "";
            form.group = (_b = props.editing.group) != null ? _b : "";
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (__props.open) {
          ...
          if (__props.open) {
            ...
            if (!__props.editing) {
              _push2(`<div><p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">Pr\xE9configurations</p><div class="flex flex-wrap gap-2"><!--[-->`);
            ...
            _push2(`<!--]--></select></div><div><div class="flex items-center justify-between mb-2"><label class="text-sm font-medium text-gray-700">En-t\xEAtes HTTP</label><button class="text-xs text-blue-600 hover:text-blue-700 font-medium"> + Ajouter </button></div>`);
            if (unref(form).headers.length > 0) {
              _push2(`<div class="space-y-2"><!--[-->`);
              ssrRenderList(unref(form).headers, (header, i) => {
                _push2(`<div class="flex gap-2 items-center"><input${ssrRenderAttr("value", header.key)} type="text" placeholder="Cl\xE9" class="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"><input${ssrRenderAttr("value", header.value)} type="text" placeholder="Valeur" class="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"><button class="p-2 text-gray-400 hover:text-red-500 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div>`);
              });
              _push2(`<!--]--></div>`);
            } else {
              _push2(`<!---->`);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(services).length === 0) {
        ...
        ssrRenderList(unref(services), (svc) => {
          ...
          _push(`</div><div class="flex items-center gap-3 mt-1 text-xs text-gray-400"><span class="truncate max-w-xs">${ssrInterpolate(svc.url)}</span><span class="shrink-0">${ssrInterpolate((_a = ADAPTER_LABELS[svc.adapter]) != null ? _a : svc.adapter)}</span><span class="shrink-0">${ssrInterpolate(svc.pollInterval)}s</span></div></div><div class="flex items-center gap-1 shrink-0"><button class="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors" title="Modifier"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>`);
      ...
      _push(`<div class="mt-8 p-4 rounded-xl bg-white border border-gray-100"><p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">L\xE9gende des niveaux</p><div class="flex flex-wrap gap-3">`);
      _push(ssrRenderComponent(_component_StatusBadge, {
        level: "operational",
        size: "sm"
      }, null, _parent));
...
//# sourceMappingURL=services-C4KEay63.mjs.map
