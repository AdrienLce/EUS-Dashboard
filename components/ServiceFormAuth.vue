...
<template>
  <div class="rounded-xl border border-gray-100 bg-gray-50 p-4 space-y-3">
    <div class="flex items-center gap-2">
      ...
      <label class="text-sm font-medium text-gray-700">Authentication</label>
    </div>
    ...
    <div v-if="auth.type === 'bearer'">
      ...
      <div class="flex gap-2">
        ...
        <button type="button" class="px-3 rounded-lg border border-gray-200 text-gray-400 hover:text-gray-600 hover:bg-white text-xs" @click="auth.showSecret = !auth.showSecret">{{ auth.showSecret ? 'Hide' : 'Show' }}</button>
      </div>
    </div>
      ...
      <div class="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <div class="flex gap-1">
            ...
            <button type="button" class="px-2 rounded-lg border border-gray-200 text-gray-400 hover:text-gray-600 hover:bg-white text-xs shrink-0" @click="auth.showSecret = !auth.showSecret">{{ auth.showSecret ? 'Hide' : 'Show' }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="auth.type === 'apikey'" class="space-y-2">
      <div>
        <label class="block text-xs text-gray-500 mb-1">Header name</label>
